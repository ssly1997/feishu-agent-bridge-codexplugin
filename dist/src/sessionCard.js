...
export function buildCodexFeatureDetailCard(feature, options = {}) {
...
function normalizeCodexFeatureKey(value) {
...
export function buildEnqueueAgentCommandActionValue(command) {
    return {
        bridge: BRIDGE_ACTION_OWNER,
...
function featureButtonRows(features) {
    const rows = [];
...
function featureActionRows(actions) {
    const rows = [];
...
function featureDetailActionsRow(refreshCommand) {
    const columns = [];
    if (refreshCommand) {
        columns.push(buttonColumn({
...
function mcpStatusBadge(status) {
    ...
    if (status === "disabled")
        return "<font color='grey'>[Disabled]</font>";
