...
export async function processNextCodexCommand(
  options: CodexCommandProcessorOptions = {}
): Promise<CodexCommandProcessResult> {
  ...
  if (control) {
    ...
    const updatedCommand = await ackCommand(
      command.id,
      controlResult.ackState,
      queuePath,
...
function codexOutputPath(config: BridgeConfig, command: AgentCommand): string {
  return join(
    config.codex.outputDir ?? join(CONFIG_DIR, "codex-output"),
    `${safeFilename(command.id)}.txt`
  );
}

function codexConfigForCommand(config: BridgeConfig, command: AgentCommand): BridgeConfig["codex"] {
  return {
    ...config.codex,
...
export function buildFeishuCommandPrompt(command: AgentCommand): string {
  const lines = [
  ...
  if (command.attachments?.length) {
    lines.push(
      "",
      "本次飞书指令包含本地附件，请直接读取这些绝对路径完成识别或分析："
    );
    for (const [index, attachment] of command.attachments.entries()) {
      lines.push(
        `${index + 1}. type=${attachment.type} path=${attachment.path}` +
...
async function notifyCommandResult(
  ...
  codex: CodexResumeResult | undefined,
  feishuClient: FeishuClient
): Promise<{ notification: "sent" | "skipped"; error?: string }> {
