...
type StdioFrameMode = "content-length" | "line";

export class MCPServer {
  private buffer = Buffer.alloc(0);
  private outputMode: StdioFrameMode = "content-length";
  ...
  private async handleFrame(body: string, mode: StdioFrameMode): Promise<void> {
    this.outputMode = mode;
    let request: JsonRpcRequest;
    try {
      request = JSON.parse(body);
    } catch (error) {
      this.writeError(null, -32700, `Parse error: ${(error as Error).message}`);
      return;
    }
  ...
  private async callTool(params: Record<string, unknown> | undefined): Promise<ToolResult> {
...
function findHeader(buffer: Buffer): { end: number; separatorLength: number } | undefined {
