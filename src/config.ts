...
export type ConfigPatch = Partial<Omit<BridgeConfig, "inbound" | "codex">> & {
  inbound?: Partial<InboundConfig>;
  codex?: Partial<CodexCliConfig>;
};
...
const receiveIdTypes = new Set<ReceiveIdType>(["chat_id", "open_id", "email"]);
const inboundModes = new Set<InboundMode>(["long_connection"]);
const codexSandboxModes = new Set<CodexSandboxMode>([
  "read-only",
  "workspace-write",
  "danger-full-access"
]);
...
const codexInProgressRecoveries = new Set<CodexInProgressRecovery>([
  "mark_failed",
  "reset_pending"
]);
...
export async function loadConfig(configPath = CONFIG_PATH): Promise<BridgeConfig> {
  let raw: string;
  try {
...
export function sanitizeConfig(config: BridgeConfig): Record<string, unknown> {
  return {
...
export function getConfigReadiness(config: BridgeConfig): Record<string, boolean> {
  return {
    enabled: config.enabled,
...
export function assertSendReady(config: BridgeConfig): void {
...
export function assertAppCredentialsReady(config: BridgeConfig): void {
...
function validateConfigShape(config: BridgeConfig, configPath: string): void {
  if (!receiveIdTypes.has(config.receiveIdType)) {
    throw new ConfigError(
      `Invalid receiveIdType in ${configPath}: ${String(config.receiveIdType)}`
    );
  }
  ...
  for (const key of ["appId", "appSecret", "receiveId", "defaultTitle"] as const) {
...
function validateInboundConfig(config: InboundConfig, configPath: string): void {
  if (!isObject(config)) {
  ...
  for (const key of ["requireMention", "acknowledgeOnReceive"] as const) {
    if (typeof config[key] !== "boolean") {
      throw new ConfigError(`inbound.${key} must be a boolean in ${configPath}`);
    }
  }

  for (const key of ["queuePath", "queueDbPath", "botOpenId", "acknowledgementText"] as const) {
    if (config[key] !== undefined && typeof config[key] !== "string") {
      throw new ConfigError(`inbound.${key} must be a string in ${configPath}`);
    }
  }

  for (const key of ["allowedChatIds", "allowedOpenIds"] as const) {
    if (
...
function validateCodexConfig(config: CodexCliConfig, configPath: string): void {
  if (!isObject(config)) {
    throw new ConfigError(`codex must be an object in ${configPath}`);
  }

  for (const key of ["enabled", "useLast", "notifyResult"] as const) {
    if (typeof config[key] !== "boolean") {
  ...
  if (!codexInProgressRecoveries.has(config.inProgressRecovery)) {
    throw new ConfigError(
      `codex.inProgressRecovery must be one of: ${Array.from(codexInProgressRecoveries).join(", ")}`
    );
  }

  if (
    config.sessionUpdatedAt !== undefined &&
  ...
  if (config.sandbox !== undefined && !codexSandboxModes.has(config.sandbox)) {
    throw new ConfigError(
      `codex.sandbox must be one of: ${Array.from(codexSandboxModes).join(", ")}`
    );
  }
  ...
  if (
    ...
    throw new ConfigError(
      `codex.approvalPolicy must be one of: ${Array.from(codexApprovalPolicies).join(", ")}`
    );
  }
...
function sanitizeInboundConfig(config: InboundConfig): Record<string, unknown> {
  return {
    enabled: config.enabled,
    mode: config.mode,
...
function sanitizeCodexConfig(config: CodexCliConfig): Record<string, unknown> {
  return {
    enabled: config.enabled,
